...
# ALL pgplot based functionalities have been commented out. Use matplotlib in future?
#import Pgplot, ppgplot
import bisect, sinc_interp
#from scipy.stats import histogram #this has been removed from scipy
...
def bins_to_accel(z, T, f=[1.0, 1000.0], device="/XWIN"):
    ...
    if (device):
        #Pgplot.plotxy(Num.log10(accels), fs, logx=1, logy=1,labx="Frequency (Hz)",laby="Acceleration (m/s\u2\d)", device=device)
        #ppgplot.pgmtxt("T", -2.0, 0.75, 0.0, "T = %.0f sec"%T)
        #ppgplot.pgmtxt("T", -3.5, 0.75, 0.0, "r\B\u\.\d = %.1f bins"%z)
        if (device != '/XWIN'):
            print("Pgplot functionality disabled")
            #Pgplot.closeplot()
    else:
        return accels
...
def smear_plot(dm=[1.0,1000.0], dmstep=1.0, subdmstep=10.0, freq=1390.0,
    ...
    #Pgplot.plotxy(Num.log10(total_smear), ldms, rangey=[minval, maxval],logx=1, logy=1, labx="Dispersion Measure",laby="Smearing (ms)", device=device)
    #ppgplot.pgsch(0.8)
...
def gauss_profile_params(profile, output=0):
    ...
    if (output):
        ...
        #Pgplot.plotxy(profile, phases, rangex=[0.0, 1.0],labx='Pulse Phase', laby='Pulse Intensity')
    ...
    if (output):
        #Pgplot.plotxy(residuals, phases, rangex=[0.0, 1.0],rangey=[min(residuals) - 2 * resid_std,max(residuals) + 2 * resid_std],labx='Pulse Phase', laby='Residuals',line=None, symbol=3)
        #ppgplot.pgerrb(6, phases, residuals,Num.zeros(len(residuals), 'd') + \resid_std, 2)
